...
allowed-tools: Bash
disable-model-invocation: false  # explicitly model-invokable — the whole point of this wrapper; must never drift to true
user-invocable: true             # explicitly available as a slash command too
---
...
```bash
bash "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/codex-adversarial-review/scripts/run_review.sh" "--scope auto focus on feasibility, completeness, missing risks, and questionable assumptions in the plan at plans/20260615-token-refresh/plan.md"
```
