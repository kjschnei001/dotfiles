...
Then, as the **very last line** of your output — after everything else — emit a copyable command to hand the plan off to implementation. It must be the final line, alone, with nothing after it, so the user can double- or triple-click to select and copy it:
    ...
    /implement-plan <target file path>

Use the **absolute path** to the plan file written in Step 8 (e.g. `/implement-plan /Users/you/code/project/plans/20260615-token-refresh/plan.md`). Emit it as a bare line — no surrounding prose, backticks, or trailing punctuation.
